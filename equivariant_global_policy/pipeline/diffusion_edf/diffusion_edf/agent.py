...
def get_models(configs_root_dir: str, 
    ...
    # print('in DiffEDFAgent:', configs_root_dir)
...
class DiffusionEdfAgent():
    ...
    def sample(self, scene_pcd: PointCloud, 
               ...
               ) -> Union[Tuple[torch.Tensor, PointCloud, PointCloud, torch.Tensor, torch.Tensor, FeaturedPoints], Tuple[torch.Tensor, PointCloud, PointCloud, torch.Tensor, torch.Tensor, FeaturedPoints, Dict[str, Any]]]:
        """
        alpha = timestep * L^2 * (t^time_exponent_alpha)
        T = temperature * (t^time_exponent_temp)
        """
        query_weights = torch.zeros(60, 1)
        query_edf = torch.zeros(60, 240)
        ...
        for model, N_steps, timesteps, temperatures, diffusion_schedules in zip(self.models, N_steps_list, timesteps_list, temperatures_list, diffusion_schedules_list):
            ...
            with torch.no_grad():
                ...
                T0 = Ts[-1]
                Ts_out.append(Ts)
                query_weights = model.score_head.query_weights
                query_edf = model.score_head.query_edf
                query_point = model.score_head.query_point
        ...
        if return_info:
            return Ts_out, scene_pcd, grasp_pcd, query_weights, query_edf, query_point, info
        else:
            return Ts_out, scene_pcd, grasp_pcd, query_weights, query_edf, query_point
