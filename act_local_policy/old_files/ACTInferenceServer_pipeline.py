...
class ACTInferenceServer(Node):
    ...
    def state_cb(self, msg): 
        print(f"[state_cb] t={time.time():.3f}")
    ...
    def service_cb(self, request, response): 
        ...
        self.load_model(self.task)
        self.reference_frame = np.array(reference_frame)
        self.get_logger().info(f"Starting inference for task: {self.task} with reference frame: {reference_frame}")
        self._timer_signal = Future()
        inference_loop = self.create_timer(self.dt, self.inference, callback_group=ReentrantCallbackGroup())

        rclpy.spin_until_future_complete(self, self._timer_signal)
        inference_loop.cancel()
        
        self.unload_model()
        self.shoot_action(self.final_pose, self.final_gains, 1)
        self.shoot_action(self.final_pose, self.final_gains, 1)
...
def main(args): 
    ...
    act_inference = ACTInferenceServer(config, debug=debug)
    try:
