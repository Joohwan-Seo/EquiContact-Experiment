...
class ACTInference(Node):
    ...
    def __init__(self, config, reference_frame = None, debug=False , collect_force=False): 
    ...
    def init_sensors(self): #NOTE(JS): Done
        ...
        for cam_name in self.camera_names:
            ...
            self.camera_cb_times[cam_name] = time.time()
    ...
    def img_cb(self, msg, camera_id): #NOTE(JS): Done
        prev_cb_time = copy.deepcopy(self.camera_cb_times[camera_id])
        self.camera_cb_times[camera_id] = time.time()
        print(f"Camera {camera_id} callback time: {self.camera_cb_times[camera_id] - prev_cb_time:.4f} seconds")
    ...
    def state_cb(self, msg): 
        ...
        # print(f"state_cb time: {time.time() - self.state_cb_time_prev}")
        self.state_cb_time_prev = time.time()
    ...
    def format_obs(self): 
        ...
        if self.collect_force:
            self.force_history.append(self.obs['FT'])
    ...
    def inference(self):
        ...
        if self.t >= self.episode_len:
            ...
            raise KeyboardInterrupt("Reached maximum timesteps, shutting down...")
    ...
    def save_FT_history(self, save_name):
        """
        Save the force history to a file.
        """
        if not self.collect_force:
            print("Force collection is not enabled.")
            return
        ...
        print(current_path)
        save_path = f"{current_path}/{save_name}"
        print(save_path)
        
        with open(save_path, 'wb') as f:
            pickle.dump(self.force_history, f)
        print(f"Force history saved to {save_path}")
...
def main(args): 
    ...
    task = "world_pose_FT_gain"
    episode = 0

    rclpy.init(args=None)
    config = yaml.load(open(config_path, "r"), Loader=yaml.Loader)
    act_inference = ACTInference(config, reference_frame=args.reference_frame, debug=debug, collect_force=collect_force)
    ...
    try:
        ...
        act_inference.exec_loop.cancel()
        act_inference.save_FT_history(f"data/{task}_{episode}.pkl")
        act_inference.destroy_node()
        rclpy.shutdown()
        print("ACT Inference Node shut down successfully.")

if __name__ == '__main__':
    import argparse 

    case = "augmented" # "default" or "augmented"
    parser = argparse.ArgumentParser()

    print(f"Running inference with case: {case}")

    if case == "default":
        parser.add_argument('--config', type=str,
                            default='/home/horowitzlab/ros2_ws_clean/src/inference_pipeline/inference_pipeline/configs/ACT_realrobot_pattern_platform_05.yaml',
                            help='Path to the config file')
    elif case == "augmented":
        parser.add_argument('--config', type=str, 
                            default='/home/horowitzlab/ros2_ws_clean/src/inference_pipeline/inference_pipeline/configs/ACT_realrobot_pattern_platform_06_distractor_tilted.yaml', 
                            help='Path to the config file')
    # parser.add_argument('--config', type=str, 
    #                     default='/home/horowitzlab/ros2_ws_clean/src/inference_pipeline/inference_pipeline/configs/ACT_realrobot_pattern_platform_05.yaml', 
    #                     help='Path to the config file')
    ...
    parser.add_argument('--reference_frame', type=float, nargs='+', default=None)
