...
class ACTInferenceServer(Node):
    ...
    def move(self):
        # print("move loop alive")
        gains = np.ones((6,)) * 1000

        if not self.pre_pick_move_finished_flag and not self.pick_finished_flag:
        ...
        elif self.pick_finished_flag and not self.grasp_move_finished_flag and not self.place_finished_flag:
    ...
    def pick(self):
        # print("pick loop alive")
        if self.pre_pick_move_finished_flag and not self.pick_finished_flag:
            self.get_logger().info("In pick task")
            task = 'pick'
            if self.temporal_agg: 
                query_freq = 1
            else:
                query_freq = self.num_queries['pick']
            ...
            with torch.inference_mode():
                ...
                if t % query_freq == 0:
                    ...
                    all_action_poses_np, all_action_gains_np, all_gripper_actions_np = self.process_actions(unnormalized_actions, 'pick') # updating all_time_poses_np and all_time_gains_np
    ...
    def place(self):
        # print("place loop alive")
        if not self.place_finished_flag and self.grasp_move_finished_flag and self.pick_finished_flag:
            self.get_logger().info("In place task")
            task = 'place'
            if self.temporal_agg: 
                query_freq = 1
            else:
            ...
            if t > self.episode_len[task]:
                ...
                self.place_loop.cancel()

            with torch.inference_mode():
                obs = self.format_obs('place')
                ...
                if t % query_freq == 0:
                    ...
                    unnormalized_actions = self.post_process[task](all_actions)
                    all_action_poses_np, all_action_gains_np, all_gripper_actions_np = self.process_actions(unnormalized_actions, 'place') # updating all_time_poses_np and all_time_gains_np
                    # print(t, all_actions[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_place', type=str, 
                        default='/home/horowitzlab/ros2_ws_clean/src/inference_pipeline/inference_pipeline/configs/ACT_realrobot_pattern_platform_04.yaml', 
