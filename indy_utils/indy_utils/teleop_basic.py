...
class IndyTeleop:
    ...
    def teleop(self):
        TCP_d_pos = np.array([0.0, 0.0, 0.0])
        TCP_d_rotm = np.eye(3)
        ...
        try:
            self.indy.start_teleop(method=TaskTeleopType.RELATIVE)
            while self.success:
                time.sleep(0.005)
                ...
                if self.home:
                    ...
                    home_rotm = R.from_euler('xyz',home_pose[3:],degrees=True).as_matrix()
                    rand_init_rotm = R.from_euler('xyz',rand_init_pose[3:],degrees=True).as_matrix()

                    target_rand_init_rotm = home_rotm.T @ rand_init_rotm

                    target_rand_init = np.zeros((6,))
                    target_rand_init[:3] = rand_init_pose[:3] - home_pose[:3] # Relative to home position (base frame)
                    ...
                    ## Teleoperation
                    self.indy.movetelel_rel(tpos = indy_cmd, vel_ratio = 0.25, acc_ratio = 4.0)
                    
                    # indy cmd (spatial frame)
                    pos = home_pose[:3] + indy_cmd[:3]
                    rotm = R.from_euler('xyz',home_pose[3:],degrees=True).as_matrix() @ TCP_d_rotm 
