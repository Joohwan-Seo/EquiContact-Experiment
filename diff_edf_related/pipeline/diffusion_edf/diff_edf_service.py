...
class DiffEDFService(Node):
    def __init__(self): 
        ...
        # self.service = self.create_service(EDF, "diff_edf_service", self.inference_diff_edf)
    ...
    def inference_diff_edf(self, request, response):
        ...
        if task == "pick": 
            ...
            if 'energy' in info.keys():
                ...
                Ts_out = Ts_out[:,3:-2] # Remove outlier energy poses
        ...
        position = result[0, 4:] * 10 # cm to mm
        quaternion = result[0, :4]
        ...
        response.position = [float(position[0]), float(position[1]), float(position[2])]  # mm in XYZ
        response.orientation = [float(orientation[0]), float(orientation[1]), float(orientation[2])]  # degrees in XYZ (deg)
