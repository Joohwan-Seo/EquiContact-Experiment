...
class DataLogger(Node): 
    ...
    def get_robot_state(self):
        ...
        obs_pos = self.indy.get_control_state()['p']
        obs_vel = self.indy.get_control_state()['pdot']
        act_data =  next((item for item in tposes['variables'] if item['addr'] == 1), None)
        act = act_data['tpos']
        return obs_pos, obs_vel, act
    
    def get_data(self):
        time = (self.get_clock().now().nanoseconds)//1000
        # time_dict = self.indy.get_int_variable()
        # time = time_dict['variables'][0]['value']

        demo = copy.deepcopy(self.demo)
        self.data[demo][time] = {}
        self.data[demo][time]['camera_id'] = {}
        for camera_id in self.camera_ids:
            camera_id = camera_id.replace("/", "")
            if self.video:
                ...
                self.data[demo][time]['camera_id'][camera_id] =  f"{self.output_dir}/visual_observations/{camera_id}/{demo}_{time}.png"

        obs_pos, obs_vel, act = self.get_robot_state()    
        self.data[demo][time]['robot_data'] = {'observations': {'obs_pos': obs_pos, 'obs_vel': obs_vel}, 'action': act}   
    
    def on_press(self, key):
        if key == KeyCode(char='t'):
            self.start_write(self.demo)
            self.get_logger().info("Starting write")
...
def main(args=None):
    ...
    parser.add_argument('--output_dir', default='/home/horowitzlab/datasets')
